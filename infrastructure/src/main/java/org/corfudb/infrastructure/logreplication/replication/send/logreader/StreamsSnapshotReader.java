...
public class StreamsSnapshotReader implements SnapshotReader {
    ...
    private LogReplicationEntryMsg generateMessage(
        ...
        LogReplicationEntryMsg txMsg = LogReplicationEntryMsg.newBuilder()
                .setMetadata(metadata)
                .setData(ByteString.copyFrom(generatePayload(opaqueEntry)))
                .build();
        ...
        log.trace("txMsg {} deepsize sizeInBytes {} entryList.sizeInByres {}  with numEntries {} deepSize sizeInBytes {}",
                TextFormat.printToString(txMsg.getMetadata()), MetricsUtils.sizeOf.deepSizeOf(txMsg), entryList.getSizeInBytes(),
                entryList.getSmrEntries().size(), MetricsUtils.sizeOf.deepSizeOf(entryList.smrEntries));
