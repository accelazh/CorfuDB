...
import static org.corfudb.protocols.CorfuProtocolCommon.generateAck;
...
public class TestDataSender implements DataSender {
    ...
    @Getter
    private Queue<LogReplicationEntryMsg> entryQueue = new LinkedList<>();
    ...
    public CompletableFuture<LogReplicationEntryMsg> send(LogReplicationEntryMsg message) {
        ...
        LogReplication.LogReplicationEntryMetadata.Builder ackMetadata =
                LogReplication.LogReplicationEntryMetadata.newBuilder().mergeFrom(message.getMetadata());
        ...
        LogReplicationEntryMsg ack = generateAck(ackMetadata.build());
        cf.complete(ack);
