...
public class NettyLogReplicationClientChannelAdapter extends IClientChannelAdapter {
    ...
    public void send(@Nonnull String endpoint, @NonNull RequestMsg msg) {
        ...
        if (channels.containsKey(endpoint)) {
            log.info("Sending message to {} on cluster {}, type={}", endpoint, getRemoteClusterDescriptor()
                    .getClusterId(), msg.getPayload().getPayloadCase());
            channels.get(endpoint).send(msg);
        } else {
            log.warn("Channel to {} does not exist, message of type={} is dropped", endpoint,
